import { Link } from 'react-router-dom';
import SCELayout from '@/components/SCELayout';
import { Shield } from 'lucide-react';

const NotFound = () => {
  return (
    <SCELayout>
      <div className="sce-section flex flex-col items-center justify-center py-12">
        <Shield className="text-sce-red w-16 h-16 mb-4" />
        <h1 className="sce-title mb-4">Доступ запрещен</h1>
        <p className="text-center mb-6 max-w-lg">
          Запрашиваемая страница не существует, или у вас недостаточный уровень допуска для просмотра данного материала.
        </p>
        <p className="text-center text-sce-darkred mb-8">
          Код ошибки: 404-UNAUTHORIZED
        </p>
        <Link to="/" className="sce-button">
          Вернуться на главную
        </Link>
      </div>
    </SCELayout>
  );
};

export default NotFound;